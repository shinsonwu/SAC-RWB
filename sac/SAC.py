from sac.utils import Actor, Double_Q_Critic
...
class SAC_countinuous():
	...
	def train(self,):
		for _ in range(5):
			...
			alpha_loss_value = alpha_loss.item() if self.adaptive_alpha else None
	...
	def save(self, EnvName, timestep, run_num):
		save_dir = f"./model/SAC_model_{run_num}"
		import os
		os.makedirs(save_dir, exist_ok=True)
		torch.save(self.actor.state_dict(), f"{save_dir}/{EnvName}_actor_{timestep}.pth")
		torch.save(self.q_critic.state_dict(), f"{save_dir}/{EnvName}_q_critic_{timestep}.pth")

	def load(self, EnvName, timestep, run_num):
		save_dir = f"./model/SAC_model_{run_num}"
		self.actor.load_state_dict(torch.load(f"{save_dir}/{EnvName}_actor_{timestep}.pth", map_location=self.dvc))
		self.q_critic.load_state_dict(torch.load(f"{save_dir}/{EnvName}_q_critic_{timestep}.pth", map_location=self.dvc))
